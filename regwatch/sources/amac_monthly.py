"""AMAC 纪律处分公告 PDF 月度爬虫（异步）。

与 :mod:`regwatch.sources.amac` 的区别：

- ``amac`` 抓取的是官网列表页里的**案例详情**（HTML 或内嵌 PDF），落库为 JSON；
- 本模块抓取的是公告**附件 PDF 原件**，按月归档到 ``AMAC_Discipline_PDFs/``，
  作为原文备份与视觉识别素材。

输出目录结构::

    AMAC_Discipline_PDFs/
        AMAC_Cases_Inst_{起始}_{结束}/    受处分机构公告
        AMAC_Cases_Person_{起始}_{结束}/  受处分人员公告
"""
...
def get_last_month_range() -> tuple[date, date]:
    ...
    logger.info(
        f"目标月份: {first_of_last_month.year}年{first_of_last_month.month}月, 范围: {first_of_last_month.date()} 至 {last_of_last_month.date()}"
    )
...
class AsyncAMACCrawler:
    ...
    async def __aenter__(self):
        connector = TCPConnector(
            limit=CONCURRENT_LIMIT, limit_per_host=CONCURRENT_LIMIT, ttl_dns_cache=300
        )

        timeout = aiohttp.ClientTimeout(total=TIMEOUT)
        self.session = ClientSession(headers=HEADERS, connector=connector, timeout=timeout)
        return self
    ...
    def _require_session(self) -> ClientSession:
        """返回已初始化的 HTTP 会话（必须在 ``__aenter__`` 之后调用）。"""
        if self.session is None:
            raise RuntimeError("AsyncAMACCrawler session is not started")
        return self.session

    async def fetch_page(self, url: str) -> str:
        """获取页面内容（指数退避重试，替代原 backoff 依赖）。"""
        ...
        logger.error("页面获取最终失败: %s", url)
        if last_error is not None:
            raise last_error
        raise RuntimeError(f"页面获取失败: {url}")
    ...
    async def download_file(self, task: DownloadTask) -> DownloadResult:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                async with self.semaphore, session.get(task.pdf_url) as response:
                    if response.status == 404 and attempt < MAX_RETRIES - 1:
                        logger.warning(
                            f"收到404，重试 {attempt + 1}/{MAX_RETRIES}: {final_filename}"
                        )
                        await asyncio.sleep(2**attempt)
                        continue
                    if response.status == 404:
                        return DownloadResult(False, final_filename, "服务器404: 文件不存在")
                ...
                last_error = e
                logger.warning(
                    f"下载中断，重试 {attempt + 1}/{MAX_RETRIES}: {final_filename} - {e}"
                )
                ...
                last_error = e
                logger.warning(
                    f"文件验证失败，重试 {attempt + 1}/{MAX_RETRIES}: {final_filename} - {e}"
                )
    ...
    async def get_pdf_from_detail(self, detail_url: str) -> str | None:
        """进入详情页获取 PDF 地址"""
        try:
            ...
            if iframe and ".pdf" in iframe_src:
                return urljoin(detail_url, iframe_src)
        ...
        with tqdm(desc=f"扫描页面 [{category_info.name}]", unit="页") as pbar:
            while not should_stop:
                ...
                try:
                    ...
                    if not items:
                        main_area = as_tag(soup.find("div", class_=re.compile("right|main")))
                        if main_area:
                            potential_items = main_area.find_all("li")
                            items = [
                                li for li in potential_items if len(li.get_text(strip=True)) > 10
                            ]
    ...
    async def execute_downloads(
        self, tasks: list[DownloadTask], category_name: str
    ) -> tuple[int, int]:
        """执行下载任务，失败后全局重试"""
        if not tasks:
        ...
        for retry_round in range(MAX_GLOBAL_RETRIES + 1):
            ...
            if retry_round > 0:
                logger.info(
                    f"{category_name}: 第 {retry_round} 轮全局重试，剩余 {len(pending_tasks)} 个文件"
                )
        ...
        logger.info(
            f"{category_name}: 下载完成 - 成功 {total_success}, 跳过 {total_skip}, 失败 {total_fail}"
        )
...
async def download_range(
    ...
    logger.info("AMAC 公告下载：%s ~ %s → %s", start_date, end_date, root)
    ...
    async with AsyncAMACCrawler() as crawler:
        for key, info in CATEGORIES.items():
            ...
            target_dir = root / f"{category_info.dir_prefix}_{start_str}_{end_str}"
            ...
            summary["failed"] += fail
            summary["dirs"][key] = target_dir.as_posix()

    elapsed = (datetime.now() - started).total_seconds()
    logger.info(
        "AMAC 公告下载完成：成功 %d，失败 %d，耗时 %.1f 秒",
        summary["success"],
        summary["failed"],
        elapsed,
    )
